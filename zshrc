...
# all virtual environments are placed here
export VIRTENVS_DIR="~/Code"

# experimental virtual environment
alias vexp="source $VIRTENVS_DIR/exp/bin/activate"
...
# Add npm to the path.
export PATH=$PATH:/usr/local/Cellar/node/5.5.0/libexec/npm/bin

# Add Qt5.2.1 to the path.
export PATH=$PATH:/usr/local/Qt5.2.1/5.2.1/clang_64/bin

# This function can be used to run framework builds of python from withing
# virtualenv, which is needed for some python applications that are not
# compatible with pip, and hence with virtualenv.
# To open a python shell:
#       frameworkpython
# To open an ipython shell:
...
# Aliases to framework'ed versions of python.
alias fripython="frameworkpython -m IPython --pylab"

export PYTHONPATH=\
"$HOME/Code/python_modules:"\
$PYTHONPATH
